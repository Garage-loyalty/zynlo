...
export function TicketList({ status, className }: TicketListProps) {
  const router = useRouter()
  const [currentPage, setCurrentPage] = useState(1)
  const { data, error, isLoading, refetch } = useTickets({ status, page: currentPage, pageSize: 25 })
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      toast.success(`${selectedTickets.size} ticket(s) succesvol verwijderd`)
      setSelectedTickets(new Set())
      refetch() // Refresh the ticket list
    } catch (error) {
      console.error('Error deleting tickets:', error)
      toast.error('Er is een fout opgetreden bij het verwijderen van tickets')
    } finally {
